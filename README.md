...
### Pure Lua (Nix-agnostic)

Use the autogenerated Lua config artifact from:

https://nightly.link/itscrystalline/sanzenvim/workflows/export-lua.yaml/main/sanzenvim-lua-linux-x86_64.zip

or

https://nightly.link/itscrystalline/sanzenvim/workflows/export-lua.yaml/main/sanzenvim-lua-linux-aarch64.zip

Unzip and copy all artifact contents into your Neovim config directory:

- Linux/macOS/Termux: `~/.config/nvim/`
- Windows: `%LOCALAPPDATA%/nvim/`

The artifact contains:

- `init.lua` (wrapper loader)
- `sanzenvim.generated.lua` (autogenerated config from nvf)
- `pack/` (plugin runtime tree)

The wrapper mirrors unwrapped startup behavior by prepending runtime/pack paths
and loading bundled plugins, while the generated Lua is rewritten to use
PATH-based tool names instead of `/nix/store/.../bin/...`.
...
Install LSPs via package manager (npm, pip, cargo, apt) and they'll be
auto-detected.
...
The bundle detects `/nix/store` and creates LSP wrappers in
`~/.cache/sanzenvim-portable/lsp-wrappers/` to access host LSPs.

### Supported LSPs

Auto-detected if in PATH: `lua-language-server`, `nil`, `rust-analyzer`,
`clangd`, `typescript-language-server`, `pyright`, `pylsp`, `jdtls`, `gopls`,
`zls`, `marksman`, `yaml-language-server`, and more.
